...
__addon__       = xbmcaddon.Addon(id='plugin.video.genesis')
__addondir__    = xbmc.translatePath( __addon__.getAddonInfo('path') )

path = os.path.join(addonPath, "item.png")
...
if mode is None:
    ...
    li = xbmcgui.ListItem('Category 1', iconImage= path)
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'folder', 'foldername': '2'})
    li = xbmcgui.ListItem('Category 2', iconImage=addonPath + '/item.png')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'folder', 'foldername': '3'})
    li = xbmcgui.ListItem('Category 3', iconImage=addonPath+'/item.png')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'folder', 'foldername': '4'})
    li = xbmcgui.ListItem('Category 4', iconImage=addonPath+'/item.png')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)
    ...
    li = xbmcgui.ListItem('Genesis', iconImage=__addondir__+'/icon.png')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = "plugin://plugin.video.mediacorp"
    __addon__       = xbmcaddon.Addon(id='plugin.video.mediacorp')
    __addondir__    = xbmc.translatePath( __addon__.getAddonInfo('path') )
    li = xbmcgui.ListItem('MediaCorp', iconImage=__addondir__+'/icon.png')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)
    url = "plugin://plugin.video.1channel"

    __addon__       = xbmcaddon.Addon(id='plugin.video.1channel')
    __addondir__    = xbmc.translatePath( __addon__.getAddonInfo('path') )
    li = xbmcgui.ListItem('1Channel', iconImage=__addondir__+'/icon.png')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)
